...
break;case'v4':if(that.doSticky(that,config)){if(that.notStickyYet(config)){that.moveElementsOnSticky(config.headerSection,config.navSection,'out',config);config.navSection.addClass('sticky-header');that.showHideElements('hide',[config.globalPromo,config.greetWelcome,config.switcherMultiStore,config.headerMultiStore]);}}else{that.moveElementsOnSticky(config.headerSection,config.navSection,'in',config);config.navSection.removeClass('sticky-header');that.showHideElements('show',[config.globalPromo,config.greetWelcome,config.switcherMultiStore,config.headerMultiStore]);}
break;default:break;}
