define(['jquery','ias','ajaxinfinitescroll'],function($){"use strict";window.ajaxInfiniteScroll={initInfiniteScroll:function(){jQuery(function($){var config={container:'.products.wrapper .product-items',item:'.product-item',pagination:'.toolbar .pages, .toolbar .limiter',next:'.pages .action.next',negativeMargin:window.negativeMargin};var pagertext=jQuery(".toolbar-amount");var toolbarCount=jQuery(".toolbar-amount .toolbar-number").length;if(toolbarCount>2){jQuery(".toolbar-amount .toolbar-number:nth-child(1)").text('1');jQuery(".toolbar-amount .toolbar-number:nth-child(2)").hide();pagertext.html(pagertext.html().replace(/\-/g,''));pagertext.html(pagertext.html().replace(/\of/g,'to'));}
...
url+='#'+hashParts[1];}
return url;},getUrlParameter:function(sParam,url){if(!url)url=window.location.href;var results=new RegExp('[\?&]'+sParam+'=([^&#]*)').exec(url);if(results==null){return 0;}else{return results[1]||0;}},replaceUrlPrameter:function(paramVal,url){var newUrl=url.replace(/(p=).*?(&|$)/,'$1'+paramVal+'$2');return newUrl;}}});