...
return $.param(paramData);},_updateContent:function(content){$(this.options.productsToolbarControl).remove();if(content.products_list){$(this.options.productsListBlock).html(content.products_list);$(this.options.productsListBlock).trigger('contentUpdated');}
if(content.filters){var isSlideIn=$('body').hasClass('slider-layer');var isSlideDown=$('body').hasClass('slider-down-layer');var isAutoClose=$('#layered-filter-block').hasClass('auto-close');var isViewL=$('body').hasClass('wp-device-l');var isViewXl=$('body').hasClass('wp-device-xl');$(this.options.layeredNavigationFilterBlock).replaceWith(content.filters);$(this.options.layeredNavigationFilterBlock).trigger('contentUpdated');if(!isAutoClose&&isSlideIn&&(isViewL||isViewXl)){$('body').addClass('wp-ln-open').css({'height':'100%','overflow':'hidden'});$('.block-search, a.logo').css({'z-index':'1'});$('.wp-ln-overlay').fadeIn(100,'swing',function(){$('.slide-in-filter').delay(150).show("slide",{direction:"left"});});}
