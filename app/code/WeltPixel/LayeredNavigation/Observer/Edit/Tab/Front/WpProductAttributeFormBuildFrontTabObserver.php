...
class WpProductAttributeFormBuildFrontTabObserver implements ObserverInterface
{
    ...
    public function categoriesArray()
    {
        ...
        foreach ($categories as $category){
            ...
            // level twoo
            $levelTwoo = $this->getChildCategories($category);
            foreach ($levelTwoo as $lvToo) {
                $catagoryList[$category->getEntityId().'_'.$lvToo->getEntityId()] = __($lvToo->getName());

                //level three
                $levelThree = $this->getChildCategories($lvToo);
                foreach ($levelThree as $lvThree) {
                    $catagoryList[$category->getEntityId().'_'.$lvToo->getEntityId().'_'.$lvThree->getEntityId()] = __($lvThree->getName());

                    //level four
                    $levelFour = $this->getChildCategories($lvThree);
                    foreach ($levelFour as $lvFour) {
                        $catagoryList[$category->getEntityId().'_'.$lvToo->getEntityId().'_'.$lvThree->getEntityId().'_'.$lvFour->getEntityId()] = __($lvThree->getName());
                    }
                }
