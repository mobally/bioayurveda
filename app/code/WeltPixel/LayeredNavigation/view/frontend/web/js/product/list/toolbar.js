define([
    ...
    $.widget('mage.productListToolbarForm', $.mage.productListToolbarForm, {
        ...
        options:
            {
                ...
                filterItemControl: '.block.filter .item a, .block.filter .filter-clear,.block.filter .swatch-option-link-layered, .pages-items a, .wp-price-slider-a',
                url: ''
            },
        ...
        _create: function () {
            ...
            /*
            var ajaxInfiniteScrollEl = $('.ajax-infinite-scroll-js');
            if(ajaxInfiniteScrollEl.length > 0) {
                $('.pages-items a').off('click');
            }
            */
        ...
        _updateContent: function (content) {
            ...
            if (content.filters) {
                ...
                if(!isAutoClose && isSlideIn && (isViewL || isViewXl)) {
                    $('body').addClass('wp-ln-open').css({'height': '100%', 'overflow': 'hidden'});
                    $('.block-search, a.logo').css({'z-index': '1'});
                    $('.wp-ln-overlay').fadeIn(100, 'swing', function () {
                        $('.slide-in-filter').delay(150).show("slide", {direction: "left"});
                    });
                }
        ...
        changeUrl: function (paramName, paramValue, defaultValue) {
            ...
            window.isSorting = (paramName == 'product_list_order') ? true : false;
