...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This Certificate no longer exists.'));
        ...
        $this->_coreRegistry->register('certificate', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Certificate') : __('New Certificate'),
            $id ? __('Edit Certificate') : __('New Certificate')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Certificate'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Certificate'));
