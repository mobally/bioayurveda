...
class PaymentIntent
{
    ...
    public function getParamsFrom($quote, $order = null, $paymentMethodId = null)
    {
        ...
        {
            $level3Data = $this->helper->getLevel3DataFrom($order, $this->config->useStoreCurrency());
            if ($level3Data)
    ...
    public function confirmAndAssociateWithOrder($order, $payment)
    {
        ...
        $paymentIntent = $this->create($params, $quote, $order); // Load or create the Payment Intent

        if (!$paymentIntent && $hasSubscriptions)
        {
