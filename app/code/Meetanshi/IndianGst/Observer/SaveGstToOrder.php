            ...
            foreach ($orderItems as $item) {
                if ($item->getQuoteItemId()) {
                    ...
                    if ($product->getGstRate()) {
                        ...
                        $sub = $gstAmount;
                        \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->info($sub);
                        if ($this->helper->canApplyTax('DIFF')) {
                            \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->info("if");
                            $item->setIgstAmount($sub);
                            $item->setBaseIgstAmount($sub);
                            $item->setIgstPercent($rate);
                        } else {
                            \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->info("else");
                            $item->setCgstAmount($sub / 2);
                            $item->setBaseCgstAmount($sub / 2);
